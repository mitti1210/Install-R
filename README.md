...
すべてのページでターミナル（macOS）や PowerShell（Windows）の開き方、コマンドの貼り付け方から丁寧に説明しています。コマンドは画面に `>` が表示されているときに入力でき、表示されていない間は処理中なので待ちます。基本的には指示されたコードをコピーして貼り付け、**Enter キー**を押せば実行できます。パスワードの入力を求められた場合、画面には文字が表示されませんが、そのまま入力して **Enter キー** を押してください。
