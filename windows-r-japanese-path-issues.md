---
layout: default
title: "Windows: ユーザー名に日本語が含まれる場合"
---
...
Windows 7/8の頃、**ユーザー名やOneDriveフォルダ名に日本語（全角文字）を含むと、Rのパッケージインストール時にエラーが頻発**する問題が報告されていました。特にライブラリの作成において、デフォルトでユーザーフォルダ配下（ドキュメントフォルダ）にライブラリを作成するため、ユーザー名やフォルダ名が日本語だとパスを正しく認識できずインストール失敗につながっていました。
...
### 過去の問題
Windows 7/8/8.1 の環境では、ユーザーフォルダ名に日本語（全角文字）が含まれると R や RStudio でパッケージをインストールできないケースが多く報告された。  

例:  
`C:\Users\日本花子\Documents\R\...` が正しく認識されず、「パスが無効」「Invalid argument」といったエラーが発生。
...
- **Microsoft アカウント**  
  ...
  → 結果として、新規環境で日本語ユーザーフォルダを目にすることは少なくなった。  
- **R 本体の改善**  
  R 4.2.0 から Windows 版は内部で UTF-8 をサポートするようになり、  
  日本語を含むパスの扱いが大幅に改善された。
- **ローカルアカウント**  
  日本語ユーザー名を入力すること自体も可能だが、あえて行わないと日本語にはならなくなった。
...
- OneDrive にリダイレクトされた「ドキュメント」などのフォルダ名に日本語が含まれると、  
  ライブラリ作成時に `…OneDrive\??????\R\win-library\…` のようなエラーが発生しパッケージの読み込みでエラーが出た。  

### 原因
- OneDrive（特に法人向け）では「OneDrive - 会社名」のように組織名に日本語が入ることがあり、それを含むパスを R が解釈できなかった。
...
- そのため発生頻度は大幅に低下した
- **条件次第では再現し得る**ため、トラブルが発生する場合は依然として「ライブラリは英数字フォルダに置く」方法も。

---

## 3. 旧OSからのアップグレード時の注意

- Windows 7/8/8.1 から Windows 10/11 にアップグレードした場合、**既存のユーザーフォルダはそのまま引き継がれる**。  
- そのため、日本語ユーザーフォルダを持っていたユーザーは、アップグレード後も引き続き日本語フォルダが残り、問題が再現する可能性がある。  
...
- **Windows 7/8/8.1からアップグレードした場合**  
  ...
  困った場合は **英数字ユーザーを新規作成してデータや管理者権限を移行**するのが一番確実。
